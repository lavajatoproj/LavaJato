...
class CustomRowTableViewCell: UITableViewCell {
    ...
    public func setupCell(product:Services2){
        ...
        self.goInHouseSwitch.isOn = product.goInHouse
        self.haveLocationSwitch.isOn = product.haveLocation
    }
    ...
    @IBAction func tappedValueSwitch(_ sender: UISwitch) {
        ...
        self.delegate?.updateCustomRowCell(index: indexPath, product: Services2(value: sender.isOn, house: self.houseSwitch.isOn, service: self.productLabel.text ?? "", price: Double(self.priceTextField.text ?? "") ?? 0.0, goInHouse: self.goInHouseSwitch.isOn, haveLocation: self.haveLocationSwitch.isOn))
    }
    
    @IBAction func tappedHouseSwitch(_ sender: UISwitch) {
        guard let cell = sender.next(CustomRowTableViewCell.self), let indexPath = cell.indexPath else { return}
        self.delegate?.updateCustomRowCell(index: indexPath, product: Services2(value: self.serviceSwitch.isOn, house: sender.isOn, service: self.productLabel.text ?? "", price: Double(self.priceTextField.text ?? "") ?? 0.0, goInHouse: self.goInHouseSwitch.isOn, haveLocation: self.haveLocationSwitch.isOn))
    }
    
    
    @IBAction func tappedServiceInLoco(_ sender: UISwitch) {
        guard let cell = sender.next(CustomRowTableViewCell.self), let indexPath = cell.indexPath else { return}
        self.delegate?.updateCustomRowCell(index: indexPath, product: Services2(value: self.serviceSwitch.isOn, house: sender.isOn, service: self.productLabel.text ?? "", price: Double(self.priceTextField.text ?? "") ?? 0.0, goInHouse: self.goInHouseSwitch.isOn, haveLocation: self.haveLocationSwitch.isOn))
        
    }
    @IBAction func tappedGoHome(_ sender: UISwitch) {
        guard let cell = sender.next(CustomRowTableViewCell.self), let indexPath = cell.indexPath else { return}
        self.delegate?.updateCustomRowCell(index: indexPath, product: Services2(value: self.serviceSwitch.isOn, house: sender.isOn, service: self.productLabel.text ?? "", price: Double(self.priceTextField.text ?? "") ?? 0.0, goInHouse: self.goInHouseSwitch.isOn, haveLocation: self.haveLocationSwitch.isOn))
    }
...
extension CustomRowTableViewCell:UITextFieldDelegate{
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        self.delegate?.updateCustomRowCell(index: indexPath, product: Services2(value: self.serviceSwitch.isOn, house: self.houseSwitch.isOn, service: self.productLabel.text ?? "", price: Double(self.priceTextField.text ?? "") ?? 0.0, goInHouse: self.goInHouseSwitch.isOn, haveLocation: self.haveLocationSwitch.isOn))
    }
