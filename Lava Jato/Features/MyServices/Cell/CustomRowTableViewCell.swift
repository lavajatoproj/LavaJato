...
class CustomRowTableViewCell: UITableViewCell {
    ...
    @IBAction func tappedValueSwitch(_ sender: UISwitch) {
        ...
        self.delegate?.updateCustomRowCell(index: indexPath, product: Services2(value: sender.isOn, house: self.houseSwitch.isOn, service: self.productLabel.text ?? "", price: self.priceTextField.text ?? "", goInHouse: self.goInHouseSwitch.isOn, haveLocation: self.haveLocationSwitch.isOn))
    }
    
    @IBAction func tappedHouseSwitch(_ sender: UISwitch) {
        guard let cell = sender.next(CustomRowTableViewCell.self), let indexPath = cell.indexPath else { return}
        self.delegate?.updateCustomRowCell(index: indexPath, product: Services2(value: self.serviceSwitch.isOn, house: sender.isOn, service: self.productLabel.text ?? "", price: self.priceTextField.text ?? "", goInHouse: self.goInHouseSwitch.isOn, haveLocation: self.haveLocationSwitch.isOn))
    }
}
...
extension CustomRowTableViewCell:UITextFieldDelegate{
    ...
    func textFieldDidEndEditing(_ textField: UITextField) {
        ...
        self.delegate?.updateCustomRowCell(index: indexPath, product: Services2(value: self.serviceSwitch.isOn, house: self.houseSwitch.isOn, service: self.productLabel.text ?? "", price: self.priceTextField.text ?? "", goInHouse: self.goInHouseSwitch.isOn, haveLocation: self.haveLocationSwitch.isOn))
    }
